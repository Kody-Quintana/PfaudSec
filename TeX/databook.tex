...
\normalsize
\hspace{13cm}\begin{tikzpicture}[overlay,scale=1.2]
\node [hex,rounded corners=3pt, fill=black] (inner) {};
\path (inner.center) node[text=white]{\fontsize{27}{27}\selectfont\textbf{Pf}};
\path (inner.center) ++({360/6*(1-0.5)}:2cm)  node[hex,fill=black,rounded corners=3pt]{} node[text=white]{\Large\textbf{Quality}}; %1
%\path (inner.center) ++({360/6*(2-0.5)}:2cm)  node[hex,fill=black,rounded corners=3pt]{} node[text=white]{\huge\textbf{2}}; %2
%\path (inner.center) ++({360/6*(3-0.5)}:2cm)  node[hex,fill=black,rounded corners=3pt]{} node[text=white]{\huge\textbf{3}}; %3
%\path (inner.center) ++({360/6*(4-0.5)}:2cm)  node[hex,fill=black,rounded corners=3pt]{} node[text=white]{\textbf{Trusted}}; %4
%\path (inner.center) ++({360/6*(5-0.5)}:2cm)  node[hex,fill=black,rounded corners=3pt]{} node[text=white]{\textbf{Dynamic}}; %5
\path (inner.center) ++({360/6*(6-0.5)}:2cm)  node[hex,fill=black,rounded corners=3pt]{} node[text=white]{\normalsize\textbf{Assurance}}; %6
\end{tikzpicture}
\LARGE
\begin{tabular}{@{}lr}
\noindent
