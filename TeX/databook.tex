...
\hspace{13cm}\begin{tikzpicture}[overlay]
\node [hex,rounded corners=2pt, fill=black] (inner) {};
\path (inner.center) node[text=white]{\huge\textbf{Pf}};
\path (inner.center) ++({360/6*(1-0.5)}:2cm)  node[hex,fill=black,rounded corners=2pt]{} node[text=white]{\textbf{Quality}}; %1
%\path (inner.center) ++({360/6*(2-0.5)}:2cm)  node[hex,fill=black,rounded corners=2pt]{} node[text=white]{\huge\textbf{2}}; %2
%\path (inner.center) ++({360/6*(3-0.5)}:2cm)  node[hex,fill=black,rounded corners=2pt]{} node[text=white]{\huge\textbf{3}}; %3
\path (inner.center) ++({360/6*(4-0.5)}:2cm)  node[hex,fill=black,rounded corners=2pt]{} node[text=white]{\textbf{Compliance}}; %4
\path (inner.center) ++({360/6*(5-0.5)}:2cm)  node[hex,fill=black,rounded corners=2pt]{} node[text=white]{\textbf{Something}}; %5
\path (inner.center) ++({360/6*(6-0.5)}:2cm)  node[hex,fill=black,rounded corners=2pt]{} node[text=white]{\textbf{Assurance}}; %6
\end{tikzpicture}
\LARGE
\begin{tabular}{@{}lr}
\noindent
