...
class MainWindow(QtGui.QMainWindow):
    ...
    #For printing to outputbox_2 (right side, for general output, see pronk function in main.py)
    ...
    #For printing to outputbox (left side, for latex output only)
    ...
    def stdoutReady(self):
        ldict = locals()
        exec('text = bytearray(self.process_' + str(self.proc_num) + '.readAllStandardOutput())',ldict)
        text = ldict['text']
    ...
    def stderrReady(self):
        ldict = locals()
        exec('text = bytearray(self.process_' + str(self.proc_num) + '.readAllStandardError())',ldict)
        text = ldict['text']
        text = text.decode("UTF-8")
        self.append(text)
